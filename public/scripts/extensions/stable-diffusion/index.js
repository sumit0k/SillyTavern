    ...
    try {
        switch (extension_settings.sd.source) {
            ...
            case sources.horde:
                ...
                break;
            case sources.vlad:
                result = await generateAutoImage(generationType, prefixedPrompt, negativePrompt, signal);
                break;
            case sources.drawthings:
 ...
 * @returns {Promise<{format: string, data: string}>} - A promise that resolves when the image generation and processing are complete.
 */
async function generateAutoImage(generationType, prompt, negativePrompt, signal) {
